 ...
 * scratch.
 */
public class VrTileTrackSelector extends MappingTrackSelector {
   ...
   */
  public VrTileTrackSelector(BandwidthMeter bandwidthMeter) {
   ...
   */
  public VrTileTrackSelector(TrackSelection.Factory adaptiveTrackSelectionFactory) {
  ...
  // MappingTrackSelector implementation.
    public void setTracksPerSource(int numTracksPerSource){
        tracksPerSource = numTracksPerSource;
    }
  @Override
  protected TrackSelection[] selectTracks(RendererCapabilities[] rendererCapabilities,
    ...
    Log.d("JOSH","In select Tracks.");
    boolean seenVideoRendererWithMappedTracks = false;
    boolean selectedVideoTracks = false;
      int numVideoRenderers = 0;

      ArrayList<Integer> videoRendererIndexes = new ArrayList<>();

    for (int i = 0; i < rendererCount; i++) {
      if (C.TRACK_TYPE_VIDEO == rendererCapabilities[i].getTrackType()) {
        Log.d("JOSH","FOUND VIDEO RENDERER");
          videoRendererIndexes.add(i);
        }
    }

    selectVideoTracks(videoRendererIndexes.toArray(new Integer[videoRendererIndexes.size()]), rendererCapabilities, rendererTrackGroupArrays,
                                     rendererFormatSupports, rendererTrackSelections, params);
...
protected void selectVideoTracks(Integer[] videoRendererIndices, RendererCapabilities[] capabilities, TrackGroupArray[] groups,
    ...
    Log.d("JOSH","tracks available: " + groupArray.length);
    for (int i = 0 ;i < videoRendererIndices.length; i++){
        Log.d("JOSH","lengths: " + groupArray.get(i).length);
        TrackGroup group = groupArray.get(i);
        int trackToPlay = 0;
        if (i > 1){
          trackToPlay = 0;
        }
        videoRendererSelections[videoRendererIndices[i]] = new FixedTrackSelection(group, trackToPlay);
    }
  ...
  protected TrackSelection selectVideoTrack(RendererCapabilities rendererCapabilities,
        ...
        Log.d("JOSH","In selectVideoTrack! " + groups.length);
        TrackSelection selection = null;
        Log.d("JOSH","In fixed Video Select " + groups.length);
  ...
  static ArrayList<Integer> alreadySelected = new ArrayList<Integer>();
    static boolean selectedOne = false;
  ...
  private static TrackSelection selectFixedVideoTrack(TrackGroupArray groups,
    ...
    int selectedPixelCount = Format.NO_VALUE;
      Log.d("JOSH","group length: " + groups.length);
      String toPrint = "";
    for (int groupIndex = 0; groupIndex < groups.length; groupIndex++) {
      ...
      int[] trackFormatSupport = formatSupport[groupIndex];
        toPrint = toPrint + groupIndex + ":" + trackGroup.length + ",";
      for (int trackIndex = 0; trackIndex < trackGroup.length; trackIndex++) {
          Log.d("JOSH","track type: " + trackIndex);
      }
    }
    Log.d("JOSH","trackGroups: " + toPrint);
