...
//      if (C.TRACK_TYPE_VIDEO == rendererCapabilities[i].getTrackType()){
        Log.d("JOSH","Renderer Type: " + rendererCapabilities[i].getTrackType());
//      }
    }
    Log.d("JOSH","Renderers in ExoPlayerImplInternal: " + renderers.length);
  ...
  private void reselectTracksInternal() throws ExoPlaybackException {
    ...
    if (selectionsChangedForReadPeriod) {
      ...
      for (int i = 0 ; i < rendererWasEnabledFlags.length; i++){
        Log.d("JOSH", "reselect renderer " + i + " type: " + renderers[i].getCapabilities().getTrackType() + " e? " + rendererWasEnabledFlags[i]);
      }
  ...
  private void setPlayingPeriodHolder(MediaPeriodHolder periodHolder) throws ExoPlaybackException {
    ...
    for (int i = 0 ; i < rendererWasEnabledFlags.length; i++){
      Log.d("JOSH", "renderer " + i + " type: " + renderers[i].getCapabilities().getTrackType() + " e? " + rendererWasEnabledFlags[i]);
    }
